...
int main(int argc, char *argv[])
{
    ...
    printf("\n");
    cout << "\033[0m[Sequential] N=" << tamano << " Wall Time : " << wallTime << " seconds \033[0m" << endl;
    cout << "\033[0;32m[Sequential] N=" << tamano << " CPU Time  : " << cpuTime  << " seconds \033[0m" << endl;
        ...
        for(int i=0;i<filA;i++){
            for(int j=0;j<colB;j++){
                for(int k=0;k<colA;k++){
                    *(*(puntero_matriz3+i)+j) += *(*(puntero_matriz1+i)+k) * *(*(puntero_matriz2+k)+j);
                } //puntero_matriz3[i][j] += puntero_matriz1[i][k] + puntero_matriz2[k][j];
            }
        }
    }
...
void guardarTiempos(int tamanoMatriz,double wallTime, double cpuTime)
{
    ...
    fileTime << "[Seq Wall Time] N=" << tamanoMatriz << " : " << wallTime << endl;
    fileTime << "[Seq CPU Time] N=" << tamanoMatriz << " : " << cpuTime << endl;
